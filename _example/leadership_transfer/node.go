...
func main() {
	var (
		...
		transferEnabled = flag.Bool("transfer", false, "leadership transfer mode")
	)
	flag.Parse()
	...
	if *transferEnabled {
		log.Printf("joined, wait 10s")

		<-time.After(10 * time.Second)

		if err := b.LeadershipTransfer(ctx); err != nil {
			log.Fatal(err)
		}
	} else {
		log.Printf("joined, wait signal")
	}
